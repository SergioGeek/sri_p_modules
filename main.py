...
if __name__ == "__main__":
	...
	for arch in listdir(confg_file["PATHS"]["ReadingPath"]):
		...
		filtro.filter()
		filtro.tokenize()
		n_tokens += len(filtro.tokens)

		fich_cleaned = open(arch_cleaned, "w")
		for tk in filtro.tokens:
	...
	print("He seleccionado el Título, Cuerpo, Ruta(Dentro de la página), Tags, Frecha y Autor")
	print("Número de ficheros procesados: " + str(n_ficheros))
	print("Número de tokens totales: " + str(n_tokens))
	print("Tokens/Fichero: " + str(n_tokens/n_ficheros))
	...
	for arch in listdir(confg_file["PATHS"]["ReadingPath"]):
		...
		filtro.tokenize()
		n_tokens += len( filtro.tokens )
		if len( filtro.tokens ) < minimo : minimo = len( filtro.tokens )
		if len( filtro.tokens ) > maximo : maximo = len( filtro.tokens )
		...
		stopper.stopping( filtro.tokens )
	...
	for arch in listdir(confg_file["PATHS"]["ReadingPath"]):
		...
		index.add( arch[0:8], stemmer.stmDic )
